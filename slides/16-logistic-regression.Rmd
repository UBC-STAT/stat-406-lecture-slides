...
* $P(Y_i = y_i \given X = x_i, \beta) = p_i^{y_i}(1-p_i)^{1-y_i}$
...
- Logistic Regression: $\prod_i f(X_i,Y_i) = \underbrace{\prod_i f(Y_i\given X_i)}_{\textrm{Logistic}}\underbrace{\prod_i f(X_i)}_{\textrm{Ignored}}$
...
__Final note:__ while logistic regression and LDA produce linear decision boundaries, they are **not** linear smoothers
