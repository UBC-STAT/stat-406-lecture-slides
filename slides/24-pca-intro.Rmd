...
1. Given $\X$, find a projection $\mathbf{P}$ onto $\R^d$ with $d \leq p$ 
...
\textrm{subject to} &\,\, \textrm{rank}(\mathbf{P}) = d,\, \mathbf{P} = \mathbf{P}^T,\, \mathbf{P} = \mathbf{P}^2
\end{aligned}
$$
The conditions ensure that $\mathbf{P}$ is a projection matrix onto $d$ dimensions.

2. Maximize the variance explained by an orthogonal transformation $\mathbf{A} \in \R^{p\times d}$
$$
\begin{aligned}
\max_{\mathbf{A}} &\,\, \textrm{trace}\left(\frac{1}{n}\mathbf{A}^\top \X^\top \X \mathbf{A}\right)\\
\textrm{subject to} &\,\, \mathbf{A}^\top\mathbf{A} = \mathbf{I}_d
\end{aligned}
$$

* In case one, the minimizer is $\mathbf{P} = \mathbf{V}_d\mathbf{V}_d^\top$
* In case two, the maximizer is $\mathbf{A} = \mathbf{V}_d$.
